...
const EditCustomer = () => {
  ...
  // Tải dữ liệu khách hàng từ server khi trang được render
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        const response = await fetch(`http://localhost:8000/v1/customer/${id}`);
        if (response.ok) {
          const data = await response.json();
          setFormData(data); // Gán dữ liệu trả về vào formData
        } else {
